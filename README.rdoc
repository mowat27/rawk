...
* <b>start {<code>}</b>: Runs before any lines are read from the input stream
** Equivalent to a BEGIN condition in awk
* <b>every {|record| <code>}</b>: Runs once for each line of input data
** Equivalent to an anonymous block (e.g. {print $1}) in awk
** every yields an object of type Line (see below)
* <b>finish {<code>}</b>: Runs after the end of the input stream
